...
for epoch in range(N_EPOCHS):
  ...
  if is_best == True:
    ...
    torch.save(model.state_dict(), MAIN_DATASET_PATH+'/models/unetvit_epoch_{}_{:.5f}.pt'.format(epoch,np.mean(val_loss_list)))
...
losses_csv = MAIN_DATASET_PATH+"/models/losses_"+str(epoch)+str(np.mean(val_loss_list))+".cvs"
